	...
	cases := []testCase{
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(wantSelectSQL).WithArgs(adminEmail).WillReturnRows(rows)
			},
			wantResult: &model.AdminUser{
		...
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(wantSelectSQL).WithArgs(adminEmail).WillReturnError(sql.ErrNoRows)
			},
		...
		},
		{
			...
			setup: func(mock sqlmock.Sqlmock) {
				mock.ExpectQuery(wantSelectSQL).WithArgs(adminEmail).WillReturnError(errBoom)
			},
			wantErrIs: errBoom,
		},
	}
